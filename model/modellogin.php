...
class modellogin extends connect{
    ...
    function ceksales($nama,$pass){
        $sqltext = "SELECT COUNT(*) FROM SALES_06992 WHERE NAMA_SALES='$nama' AND PASSWORD='$pass'";
    ...
    function akunsales($nama,$pass){
        $sqltext = "SELECT * FROM SALES_06992 WHERE NAMA_SALES='$nama' AND PASSWORD='$pass'";
        $statement = oci_parse($this->koneksi,$sqltext);
        oci_execute($statement);
        $key=oci_fetch_array($statement,OCI_BOTH);
        $this->akunsales=$key;
    ...
    function cekkasir($nama,$pass){
        $sqltext = "SELECT COUNT(*) FROM KASIR_06992 WHERE NAMA_KASIR='$nama' AND PASSWORD='$pass'";
        $statement = oci_parse($this->koneksi,$sqltext);
        oci_execute($statement);
        $key=oci_fetch_array($statement,OCI_BOTH);
    ...
    function akunkasir($nama,$pass){
        $sqltext = "SELECT * FROM KASIR_06992 WHERE NAMA_KASIR='$nama' AND PASSWORD='$pass'";
        $statement = oci_parse($this->koneksi,$sqltext);
        oci_execute($statement);
        $key=oci_fetch_array($statement,OCI_BOTH);
        $this->akunkasir=$key;
